...
def sphere_intersection_point( ray, sphere ):
   ...
   raySphere = vector_from_to( sphere.center, ray.pt )
...
def find_closest_sphere( sphere_list, ray ):
   intersectPoints= find_intersection_points( sphere_list, ray )
   if intersectPoints == []:
      return None
   closest_sphere = None
   shortestdis = length_to_from( intersectPoints[0][1], ray.pt )
   for tupl in intersectPoints:
      currentdis = length_to_from( tupl[1], ray.pt )
...
def hidden_sphere( sphere_list, point, light, Ldir ):
   ...
   for p in range( 1, len( intersectPoints ) ):
      currentdis = length_to_from( intersectPoints[p][1], ray.pt )
...
def specular_value( eyepoint, true_point, sphere, light, lightdiffuse ):
   true_normal= sphere_normal_at_point( sphere, true_point )
   fake_point = translate_point( true_point, true_normal * 0.01  )
   Ldir = normalize_vector( vector_from_to( fake_point, light.pt ) )
   reflectvec = difference_vector( true_normal * lightdiffuse * 2, Ldir )
   spectvec = normalize_vector( vector_from_to( fake_point, eyepoint ) )
